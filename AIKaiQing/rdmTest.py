...
df_selected = df.loc[:, selected_columns].copy()  # Use .loc to avoid SettingWithCopyWarning

# Convert 'host_since' to datetime
df_selected['host_since'] = pd.to_datetime(df_selected['host_since'], errors='coerce')

# Convert 'review_scores_rating' to numeric and fill missing values
df_selected['review_scores_rating'] = pd.to_numeric(df_selected['review_scores_rating'], errors='coerce')
df_selected['review_scores_rating'] = df_selected['review_scores_rating'].fillna(df_selected['review_scores_rating'].mean())

# Group by 'host_since' and calculate average 'review_scores_rating'
df_selected = df_selected[df_selected['host_since'].notnull()]
review_scores_over_time = df_selected.groupby(df_selected['host_since'].dt.to_period('M')).mean()['review_scores_rating']
